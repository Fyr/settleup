...
class Application_Model_Grid_Callback_ActionSettlementPayments
{
    ...
    public function body()
    {
        if ($this->row['settlement_cycle_status'] != Application_Model_Entity_System_SettlementCycleStatus::APPROVED_STATUS_ID && Application_Model_Entity_Accounts_User::getCurrentUser(
            ...
            return '<a class="btn btn-primary" href="/payments_payments/edit/id/' . $this->row['id'] . '?back=' . urlencode(
                'settlement_index/contractor/id/' . $this->row['contractor_id']
            ) . '"><i class="icon-pencil icon-white"></i>&nbsp;Edit</a>' . ' <a class="btn btn-danger confirm' . ($this->row['recurring'] ? ' recurring' : '') . '" data-confirm-description-title="Deleting" confirm-type="Deletion" href="/payments_payments/delete/id/' . $this->row['id'] . '?back=' . urlencode(
                'settlement_index/contractor/id/' . $this->row['contractor_id']
