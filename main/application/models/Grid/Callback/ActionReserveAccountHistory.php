...
class Application_Model_Grid_Callback_ActionReserveAccountHistory
{
    ...
    public function body()
    {
        if ($this->view->gridModel->getSettlementCycle()->getStatusId(
            ...
            return '<a class="btn btn-primary" href="/reserve_accountpowerunit/view/id/' . $this->row['reserve_account_contractor_id'] . '?back=' . urlencode(
                'settlement_index/contractor/id/' . $this->row['contractor_id']
            ) . '"><i class="icon-search icon-white"></i> View</a>';
        } else {
            return '<a class="btn btn-primary" href="/reserve_accountpowerunit/edit/id/' . $this->row['reserve_account_contractor_id'] . '?back=' . urlencode(
                'settlement_index/contractor/id/' . $this->row['contractor_id']
